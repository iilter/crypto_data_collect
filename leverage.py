...
from classlib import Leverage as leverageClass
...
def main(exchange, window, from_date, to_date, limit, return_format):
    ...
    url = quantConfig["url_estimated_leverage_ratio"]
    accessToken = quantConfig["access_token"]

    api = leverageClass()
    api.cursor = cursor
    records = api.getData(url, accessToken, exchange, window, from_date, to_date, limit, return_format)
    if (records is not None) and (len(records) > 0):
        ...
        for record in records:
            ...
            api.indexDate = dt
            api.estimatedLeverageRatio = record["estimated_leverage_ratio"]
            api.addData()
