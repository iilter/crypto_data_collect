...
from classlib import Inflow as inflowClass
...
def main(exchange, window, from_date, to_date, limit, branch, return_format):
    ...
    url = quantConfig["url_inflow"]
    accessToken = quantConfig["access_token"]

    api = inflowClass()
    api.cursor = cursor
    records = api.getData(url, accessToken, exchange, window, from_date, to_date, limit, branch, return_format)
    if (records is not None) and (len(records) > 0):
        ...
        for record in records:
            ...
            api.indexDate = dt
            api.inflowTotal = record["inflow_total"]
            api.inflowTop10 = record["inflow_top10"]
            api.inflowMean = record["inflow_mean"]
            api.inflowMeanMa7 = record["inflow_mean_ma7"]
            api.addData()
